...
export async function runPythonCode(code: string) {
    ...
    try {
        const pythonServiceUrl = process.env.PYTHON_SERVICE_URL || "http://python:5000";

        console.log(`[${new Date().toISOString()}] Executing Python code (${code.length} bytes)`);
        ...
        if (!response.ok) {
            console.error(`[${new Date().toISOString()}] Python service error: ${response.status}`);
            ...
            return {
                success: false,
                output: "",
                error: errorData.error || `Python service error: ${response.statusText}`,
            };
        }
        ...
        if (error.cause?.code === 'UND_ERR_SOCKET') {
            errorMessage = "Python service connection closed. The code may have caused a crash or timeout.";
