...
export async function runPySparkCode(code: string, inputs?: any) {
    ...
    try {
        const pysparkServiceUrl = process.env.PYSPARK_SERVICE_URL || "http://pyspark:5005";

        console.log(`[${new Date().toISOString()}] Executing PySpark code (${code.length} bytes)`);
        ...
        if (!response.ok) {
            ...
            return {
                success: false,
                output: "",
                error: errorData.error || `PySpark service error: ${response.statusText}`,
            };
        }
