...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const avgCompletion = parseFloat(completionResult.rows[0].completion_rate || 0);
        ...
        `;
        const lastMonthCompletionResult = await pool.query(lastMonthCompletionQuery);
        const lastMonthCompletion = parseFloat(lastMonthCompletionResult.rows[0].completion_rate || 0);
        ...
        const examsChange = lastMonthExams > 0
            ? ((activeExams - lastMonthExams) / lastMonthExams * 100).toFixed(1)
            : activeExams > 0 ? "100.0" : "0.0";

        const questionsChange = lastMonthQuestions > 0
            ? ((totalQuestions - lastMonthQuestions) / lastMonthQuestions * 100).toFixed(1)
            : totalQuestions > 0 ? "100.0" : "0.0";

        const completionChange = lastMonthCompletion > 0
            ? ((avgCompletion - lastMonthCompletion) / lastMonthCompletion * 100).toFixed(1)
            : avgCompletion > 0 ? "100.0" : "0.0";
        ...
        res.status(200).json({
            stats: {
                totalUsers,
                usersChange: parseFloat(usersChange) >= 0 ? `+${usersChange}%` : `${usersChange}%`,
                usersTrend: parseFloat(usersChange) >= 0 ? 'up' : 'down',
                activeExams,
                examsChange: parseFloat(examsChange) >= 0 ? `+${examsChange}%` : `${examsChange}%`,
                examsTrend: parseFloat(examsChange) >= 0 ? 'up' : 'down',
                totalQuestions,
                questionsChange: parseFloat(questionsChange) >= 0 ? `+${questionsChange}%` : `${questionsChange}%`,
                questionsTrend: parseFloat(questionsChange) >= 0 ? 'up' : 'down',
                avgCompletion: avgCompletion.toFixed(1),
                completionChange: parseFloat(completionChange) >= 0 ? `+${completionChange}%` : `${completionChange}%`,
